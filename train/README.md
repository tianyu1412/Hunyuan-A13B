...
Hunyuan-A13B provides processes related to model training. This section details how to process training data for model training purposes.

## Training Data Format and Processing

**Hunyuan-A13B-Instruct supports both "slow thinking" and "fast thinking" modes. By default, the model outputs in slow thinking mode. If you wish the model to use fast thinking, you can prepend "/no_think" to the query.**
...
You can quickly get started by following the instructions in the [Quick Start Guide](../examples/README_EN.md).
...
Based on testing, when `make_moe_param_leaf_module` and `zero3+offload` are disabled and `max_seq_length` is set to 2048, full fine-tuning requires at least a single machine with 8 GPUs (each with at least 80GB of memory).
...
If you are familiar with LLaMA-Factory, you may use [https://github.com/hiyouga/LLaMA-Factory/tree/main](https://github.com/hiyouga/LLaMA-Factory/tree/main) for fine-tuning. We provide a sample configuration file for LLaMA-Factory training at `./train/llama_factory_support/hunyuan_a13b_full_sft.yaml`.