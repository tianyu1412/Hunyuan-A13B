# Named Entity Recognition Task Example

To help users get started quickly, we have prepared a real-world example demonstrating how to fine-tune `Hunyuan-A13B`.

- Base model: `Hunyuan-A13B-Instruct`
- Training data: Approximately 5k [entity extraction samples](data) from the automotive domain
- An example of the training data is as follows:
```
{"messages": [{"role": "system", "content": "You are a helpful assistant."}, {"role": "user", "content": "/no_think Please extract the car series mentioned in the following article:\n\"Warm family man\" The changes and constants of Honda CR-V\n"}, {"role": "assistant", "content": "<think>\n\n</think>\n<answer>\nHonda CR-V\n</answer>"}]}
```

## Training

- For environment setup, please refer to the Model Training section in [README.md](../README.md).
- For the training script, refer to [train_demo.sh](train_demo.sh). You will need to modify the `model_path`, `train_data_file`, and `output_path` parameters.
- The initial model loading may take some time. Once running successfully, the loss for each step will be printed.
...
- For inference environment setup, please refer to the Inference and Deployment section in [README.md](../README.md)

## Inference

- For the inference script, refer to [eval_demo_vllm.py](eval_demo_vllm.py)
- For the evaluation script, refer to [compute_acc.py](compute_acc.py)